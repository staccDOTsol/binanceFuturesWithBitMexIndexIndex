...
async function cancelallbyorderstatus() {
    ords = await client.fetchOpenOrders('ETH/USDT')
...
setInterval(async function() {
    ...
    if (first) {
        ...
        ohlcv = await client2.fetchOHLCV('ETH/USDT', timeframe = rsiTF.toString() + 'm', since = undefined, limit = 74, params = {})
        //console.log(ohlcv)
        var c = 0;
        for (var b in ohlcv) {
            ohlcvs.push[ohlcv[b]]
        ...
        ohlcv = await client2.fetchOHLCV('ETH/USDT', timeframe = mfiTF.toString() + 'm', since = undefined, limit = 1000, params = {})
        //console.log(ohlcv)
        high = []
        low = []
        close = []
    ...
    if (pos[0] != undefined) {
        ...
        if (unrealized > takeProfit) {
            if (position > 0) {
                ...
                signal = { key: bva_key, stratname: 'ETH bit.ly/NeoNBot', pair: 'ETHUSDT', sell_price: LB-100} 
            socket_client.emit("sell_signal", signal)
            } else {
                signal = { key: bva_key, stratname: 'ETH bit.ly/NeoNBot', pair: 'ETHUSDT', buy_price: HA + 100}
...
var theMFI = []
setInterval(async function() {
    ohlcv = await client2.fetchOHLCV('ETH/USDT', timeframe = rsiTF.toString() + 'm', since = undefined, limit = 1000, params = {})
    //console.log(ohlcv)
    var c = 0;
    for (var b in ohlcv) {
        ohlcvs.push[ohlcv[b]]
...
async function doit() {
    if (keygood){
            ...
            signal = { key: bva_key, stratname: 'ETH bit.ly/NeoNBote', pair: 'ETHUSDT', sell_price: prc} 
            socket_client.emit("sell_signal", signal)
                        //console.log(openorders)
                ...
                if (buying == 0 && (freePerc < maxFreePerc || position < 0)) {
                    ...
                    if (dobuy) {
                        ...
                        signal = { key: bva_key, stratname: 'ETH bit.ly/NeoNBot', pair: 'ETHUSDT', buy_price: prc}
                        socket_client.emit("buy_signal", signal)
